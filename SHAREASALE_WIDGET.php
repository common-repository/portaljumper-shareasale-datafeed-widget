<?php
/*
Plugin Name: linksalt Shareasale datafeed widget
Plugin URI: http://portaljumper.com
Description: This plugin places widgets that will automatically pull affiliate posts out of YOUR Shareasale merchant stores. Generate extra income !! Simply enter your affiliate ID in the widget area and place the widget. <strong><font color='red'>Visit <a href='http://linksalt.com'>linksalt.com</a> for more widgets, info and tips.</font></strong> or <a href="http://www.shareasale.com/r.cfm?b=69&u=438368&m=47&urllink=&afftrack=" target="_blank">join shareasale here.</a>
Version: 4.1
Author: Peter scheepens
Author URI: http://linksalt.com
*/
add_action( 'widgets_init', 'pj_SAS_widget' );

function pj_SAS_widget() {
	register_widget( 'portaljumper_shareasale_widget' );
}
...
class portaljumper_shareasale_widget extends WP_Widget 
{


function portaljumper_shareasale_widget() 
{
	$widget_ops = array( 'classname' => 'ls_ex1', 'description' => __('This widget will automatically show affiliate advertisement from your shareasale network. Earn affiliate-income on auto-pilot. Automatic datafeed extractors by portaljumper.com & linksalt.com', 'ls_ex1') );
	$control_ops = array( 'width' => 900, 'height' => 350, 'id_base' => 'ls_ex1-widget' );
	$this->WP_Widget( 'ls_ex1-widget', __('linksalt.com Shareasale widgets', 'ls_ex1'), $widget_ops, $control_ops );
}
...
function widget( $args, $instance ) 
{
		...
		$affid = get_option('ls_SAS_name');
		$merchant = get_option('ls_SAS_merch');
		$size = get_option('ls_SAS_size');
		echo $before_widget;
		if ( $title )
		echo $before_title . $title . $after_title;

// begin on screen info	
$preview_size = get_option('ls_SAS_size');
$psize = explode ("-",$preview_size);
?>
<iframe src="http://linksalt.com/admaker.php?affid=<?PHP echo get_option('ls_SAS_name'); ?>&feed=<?PHP echo get_option('ls_SAS_merch'); ?>&searchword=&adsize=<?PHP echo get_option('ls_SAS_size'); ?>&network=shareasale&form2=1&co1=FFFFFF&co2=FFFFFF" width="<?PHP echo $psize[0]; ?>" Height="<?PHP echo $psize[1]; ?>" scrolling='no' frameborder='0' marginheight='0' marginwidth='0'>linksalt.com embedding the worlds media</iframe>
<?php
// end on screen info
echo $after_widget;
}
...
function update( $new_instance, $old_instance ) {
		...
		update_option('ls_SAS_title',$instance['title']);
		update_option('ls_SAS_name',$instance['name']);
		update_option('ls_SAS_merch',$instance['merch']);
		update_option('ls_SAS_size',$instance['size']);
		return $instance;
	}
...
{
		$defaults = array( 'title' => __('linksalt.com tools', 'ls_ex1'), 'name' => __('438368', 'ls_ex1'), 'merch' => __('FitnessGearUsa_Com', 'ls_ex1'));
		...
		$ch = curl_init(); 		
		curl_setopt($ch,CURLOPT_URL,'http://linksalt.com/getfeeds.php?network=shareasale');
		...
		This widget is for the Shareasale affiliate program.<br> <a href="http://www.shareasale.com/r.cfm?b=69&u=438368&m=47&urllink=&afftrack=" target="_blank">Sign up for Shareasale</a> | 
		<a href="http://myplugin.eu" target="_blank">Get widgets for other networks ..</a><hr>
		The front end is now showing widgets from Shareasale merchant: <font color='red'><?php echo $instance['merch']; ?></font>.<br>Pleae make sure you are affiliated with this merchant
		<br> <a href="http://www.shareasale.com/r.cfm?b=69&u=438368&m=47&urllink=&afftrack=" target="_blank">Add this merchant to your campaign now.</a><hr>
		</div>
